-- Enhanced Chat System Database Schema (Safe Version - Handles Existing Data)
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'workspace_chats' 
                 AND column_name = 'message_type') THEN
    ALTER TABLE public.workspace_chats ADD COLUMN message_type TEXT DEFAULT 'text';
    ALTER TABLE public.workspace_chats ADD CONSTRAINT workspace_chats_message_type_check 
      CHECK (message_type IN ('text', 'file', 'code', 'image'));
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE public.workspace_chats ADD COLUMN file_url TEXT;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE public.workspace_chats ADD COLUMN file_name TEXT;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE public.workspace_chats ADD COLUMN file_size INTEGER;
  END IF;
END $$;
...
DO $$ 
BEGIN
  -- Drop workspace_group_chats policies if they exist
  DROP POLICY IF EXISTS "Users can view group chats in workspaces they belong to" ON public.workspace_group_chats;
  DROP POLICY IF EXISTS "Users can send group chats in workspaces they belong to" ON public.workspace_group_chats;
  
  -- Drop chat_notifications policies if they exist
  ...
  DROP POLICY IF EXISTS "System can create notifications for users" ON public.chat_notifications;
END $$;

-- Create workspace group chats policies
...
SELECT 'Enhanced Chat System setup completed successfully!' as status;
